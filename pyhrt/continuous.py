...
def fit_mdn(X, y, ncomponents=5,
    ...
    # tmp_model_filename = 'best_mdn_model_' + str(uuid.uuid4()) + '.pt'
    # tmp_file_for_best_model = os.path.join(temp_dir, tmp_model_filename)
    # ---
    temp_directory = tempfile.gettempdir() # Gets an OS-appropriate temp directory
    # Create a unique filename in that directory
    tmp_file_path = os.path.join(temp_directory, 'tmp_mdn_model_' + str(uuid.uuid4()) + '.pt') 
    ...
    for epoch in range(nepochs):
        ...
        if epoch == 0 or val_losses[epoch] <= best_loss:
            ...
            torch.save(model, tmp_file_path)
        
        if verbose:
            print('Validation loss: {} Best: {}'.format(val_losses[epoch], best_loss))

    model = torch.load(tmp_file_path,weights_only=False)
